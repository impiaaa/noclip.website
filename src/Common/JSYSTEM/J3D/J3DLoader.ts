...
function readMAT3Chunk(buffer: ArrayBufferSlice): MAT3 {
    ...
    for (let i = 0; i < materialCount; i++) {
        ...
        */
        ...
        const hasIndirectTable = indirectTableOffset !== 0 && indirectTableOffset !== nameTableOffs;
        if (hasIndirectTable) {
    ...
    function readColorChannel(colorChanIndex: number): GX_Material.ColorChannelControl {
        if (colorChanIndex !== 0xFFFF) {
            ...
            const attnLookup = [GX.AttenuationFunction.SPEC, GX.AttenuationFunction.SPOT, GX.AttenuationFunction.NONE, GX.AttenuationFunction.NONE]; // last two are actually DIR
